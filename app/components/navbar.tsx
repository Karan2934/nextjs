...
export default function Navbar() {
    ...
    return (
        ...
        <div>
            <div className="p-7">
                <div className="bg-white w-full rounded-3xl flex items-center justify-between max-h-27">
                    ...
                    <div className="hidden lg:flex items-center space-x-10 text-[#1b152c80] ml-20 font-medium text-base">
                        {navData.map((navBar: any) => (
                            <div key={navBar.title} className="relative group">
                                <Link href={navBar.slug} className="hover:text-[#6c43e3] " >{navBar.title.toUpperCase()}
                                    <span className=""></span>
                                </Link>
                                                ...
                                                {subNav.child && (
                                                    <div className="absolute left-full top-0 bg-white shadow-lg p-4 opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-200 min-w-48">
                                                        {subNav.child.map((subSubNav: any) => (
                                                            <Link key={subSubNav.title} href={subSubNav.slug} className="block py-2 px-4 hover:bg-gray-100 cursor-pointer">
                                                                {subSubNav.title}
                                                            </Link>
                                                        ))}
                    ...
                    <div className="flex ml-auto">
                        <div className="flex items-center space-x-4 p-5">
                            <h1 className="text-[#1b152c80] scale-x-75 scale-y-250 ">|</h1>
                        ...
                        <div className="hidden xl:block max-w-70 bg-[#1b152c] h-27 p-10 rounded-r-3xl">
